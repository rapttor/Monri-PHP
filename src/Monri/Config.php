...
class Config
{
    ...
    public static function getStaticMerchantKey()
    {
        return isset($_ENV['SDK_MONRI_KEY']) ? $_ENV['SDK_MONRI_KEY'] : null;
    }

    public function getStaticAuthenticityToken()
    {
        return isset($_ENV['SDK_MONRI_TOKEN']) ? $_ENV['SDK_MONRI_TOKEN'] : null;
    }

    public static function calculateDigest(string $data, ?string $key)
    {
        if (!isset($key) || $key == null)
            $key = self::getStaticMerchantKey();
        return hash('SHA1', $key . $data);
    }
